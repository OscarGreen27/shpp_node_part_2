 ...
 */
async function getNamePromiseAll(urls: string[]): Promise<string[]> {
  const result: string[] = [];
 ...
 * @returns - array of usernames
 */
async function getNameAsyncAwait(urls: string[]): Promise<string[]> {
  const result: string[] = [];
  try {
    for (let i = 0; i < urls.length; i++) {
      const resp: Response = await fetch(urls[i]);
...
function getNamePromise(urls: string[]) {
  const result: string[] = [];

  function getName(urls: string): Promise<string> {
  ...
  const name: Promise<string> = getName(urls[0]);
  console.log(name);
}
 ...
 */
function findName(str: string): string {
  const regx: RegExp = /"name":\s*([^,]+)/;
